...
// Init Resources
_Font* Sexy::FONT_DEFAULT;
_Font* Sexy::FONT_HUNGARR;
...
_Font* Sexy::GetFontById(int theId)
{
	return *(_Font**)gResources[theId];
}
...
static Sexy::ResourceId GetIdByVariable(const void *theVariable)
{
	...
	MyMap::iterator anItr = aMap.find((int)(size_t)theVariable);
	if (anItr == aMap.end())
		return RESOURCE_ID_MAX;
	else
		return (ResourceId) anItr->second;
