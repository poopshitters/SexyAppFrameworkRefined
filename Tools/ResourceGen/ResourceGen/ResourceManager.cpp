...
std::string ResourceManager::GetTypeName(const BaseRes& theRes)
{
    ...
    case ResType_Sound:
        return "int";
    case ResType_Font:
        return mUsingPatoke ? "_Font*" : "Font*";
    }
...
void ResourceManager::WriteSourceFileGroup(FILE* theFile, const std::string& theResGroup, ResMap& theMap)
{
    ...
    for (ResMap::iterator anItr = theMap.begin(); anItr != theMap.end(); ++anItr) {
        ...
        switch (aRes.mType) {
        case ResType_Image:
            aMethodName = "GetImageThrow";
            break;
        case ResType_Sound:
            aMethodName = "GetSoundThrow";
            break;
        case ResType_Font:
            aMethodName = "GetFontThrow";
            break;
        }
...
void ResourceManager::WriteSourceFile(FILE* theFile, const std::string& theFileName)
{
    ...
    if (mUsingPatoke)
        fprintf(theFile, "#include \"SexyAppFramework/misc/ResourceManager.h\"\n");
    fprintf(theFile, "\nusing namespace Sexy;\n\n");
    ...
    if (mUsingPatoke)
    {
        fprintf(theFile, "_Font* Sexy::%sGetFontById(int theId)\n", mFunctionPrefix.c_str());
        fprintf(theFile, "{\n");
        fprintf(theFile, "\treturn *(_Font**)gResources[theId];\n");
        fprintf(theFile, "}\n\n");
    }
    else
    {
    ...
    if (mUsingPatoke)
    {
        fprintf(theFile, "_Font*& Sexy::%sGetFontRefById(int theId)\n", mFunctionPrefix.c_str());
        fprintf(theFile, "{\n");
        fprintf(theFile, "\treturn *(_Font**)gResources[theId];\n");
        fprintf(theFile, "}\n\n");
    }
