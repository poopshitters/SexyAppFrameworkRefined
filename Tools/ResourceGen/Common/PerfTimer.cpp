...
struct PerfInfo
{
	...
	PerfInfo(const char *theName) : mPerfName(theName), mStartTime(0), mDuration(0), mStartCount(0), mCallCount(0) { }
	...
	else
	{
		...
		if(anItr != gPerfInfoSet.end())
		{
			if( --anItr->mStartCount == 0)
				anItr->mDuration += theRecord.mTime - anItr->mStartTime;		
		}
	}
}
...
std::string SexyPerf::GetResults()
{
	...
	{
		const PerfInfo &anInfo = *anItr;
		sprintf(aBuf,"%s (%d calls, %%%.2f time): %.2f (%.2f avg)\n",anInfo.mPerfName,anInfo.mCallCount,anInfo.mMillisecondDuration/gDuration*100,anInfo.mMillisecondDuration,anInfo.mMillisecondDuration/anInfo.mCallCount);
		aResult += aBuf;
	}
